# ARCHIVE — Reconstruction de l'interface VibeCut (2026-07-29 → 2026-08-04)

> **Chantier CLOS le 2026-08-04. Ce document est une archive de référence, pas
> une liste de tâches.** Il a été sorti de `todo.md` le 2026-08-08 pour que le
> TODO ne porte plus que le chantier actif (redesign VibeOS).
>
> Résultat final : 48/48 transitions et 12/12 mouvements rendus des deux côtés,
> export serveur déployé (révision `vibecut-render-service-00016-thk`), aucun
> écart aperçu ↔ production. Interface unique sur `/video`.
>
> **À quoi ça sert encore** : les leçons payées (pièges FFmpeg, parité canvas ↔
> serveur, méthode de mesure), les bugs 1 à 59 avec leurs causes réelles, les
> commandes de test VibeCut, et les problèmes connus non résolus (§ « Problèmes
> connus »). À relire avant toute reprise de `/video` ou de `render-service/`.
>
> Chantier actif : [todo.md](../todo.md) · Plan VibeCut : [plan.md](../plan.md)
...
## Prompt de relance VibeCut — PÉRIMÉ (conservé pour la méthode)

> ⚠️ Ce prompt visait le contenu des bibliothèques, **livré depuis** (48/48 et
> 12/12). Ne pas le rejouer tel quel. Il reste ici pour ses sections « LEÇONS
> DÉJÀ PAYÉES », « RÈGLES NON NÉGOCIABLES » et « ÉCHECS DE TESTS PRÉEXISTANTS »,
> qui restent vraies.

### Texte d'origine (2026-08-04)
...
*(Le chantier actif — redesign VibeOS — vit dans [todo.md](../todo.md).)*
